...
class verge_content(manage):
    def __init__(self):
        self.data = []
        self.mn = manage('https://www.theverge.com/games')
        ...
        for i in r_external:
            try:
                ...
                pass
        self.ref_external =list(filter(('www.theverge.com').__ne__, self.s_external))
        print(Counter(self.ref_external))
        # print(len(self.s_external))
        # print('++++++++++++',external)
        print('crawl done')

        pattern =  re.compile(r'(https://www.theverge.com/\d+/\d+/\d+/\d+/)')
    ...
    def get_content(self,i):
        ...
        for k in soup.find_all('div',{'class': re.compile(r'(c-entry-content)')}):
