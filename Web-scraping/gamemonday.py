...
class gamemonday_content(manage):
    def __init__(self):
        self.data = []
        self.mn = manage('https://www.gamemonday.com/')
        ...
        for i in r_external:
            try:
                ...
                pass
        self.ref_external =list(filter(('www.gamemonday.com').__ne__, self.s_external))
        print(Counter(self.ref_external))
        # print(len(self.s_external))
        # print('++++++++++++',external)
        print('crawl done')
        
        pattern =  re.compile(r'(https://www.gamemonday.com/game-news/\d{6}/)')
        ...
        r = requests.get('https://www.gamemonday.com/').text
    ...
    def get_content(self,i):
        ...
        for k in soup.find_all('div',{'class': re.compile(r'(entry-content mb-4)')}):
            for p in k.find_all('p'):
                for j in p:
                    text = text + j.text.replace('\n','').replace('\ ','').strip()
        if text == '':
            ...
            polarity = self.mn.check_sentiment(text,'en')
