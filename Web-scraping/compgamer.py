...
class compgamer_content(manage):
    def __init__(self):
        print('start crawl')
        self.data = []
        self.mn = manage('https://www.compgamer.com/mainpage/')
        ...
        for i in r_external:
            try:
                ...
                pass
        self.ref_external =list(filter(('www.compgamer.com').__ne__, self.s_external))
        print(Counter(self.ref_external))
        # print(len(self.s_external))
        # print('++++++++++++',external)
        print('crawl done')
        pattern = re.compile(r'(https://www.compgamer.com/mainpage/\d{6}/)')
        ...
        r = requests.get('https://www.compgamer.com/mainpage/').text
    ...
    def get_content(self, i):
        start = time.time()
        #print(len(self.new_non_dup_url))
        obj = {}
        text = ''
        start2 = time.time()
        req = self.mn.get_req(i)
        #print('after get req ',time.time()-start2)
        self.soup = self.mn.get_soup(req)
        obj['name'] = i.replace('https://www.','').split('.')[0]
        obj['date'] = datetime.today().strftime('%Y/%m/%d')
        obj['url'] = i 
        for header in self.soup.find_all('h1'):
            obj['header'] = header.text.replace('\n','').replace('\ ','').strip()
        
        for k in self.soup.find_all('div',{'class': re.compile(r'(entry clearfix)')}):
