...
class tech_content(manage):
    def __init__(self):
        self.data = []
        self.mn = manage('https://tech.hindustantimes.com/gaming/')
        ...
        for i in r_external:
            try:
                ...
                pass
        self.ref_external =list(filter(('tech.hindustantimes.com').__ne__, self.s_external))
        print(Counter(self.ref_external))
        # print(len(self.s_external))
        # print('++++++++++++',external)
        print('crawl done')
        
        pattern =  re.compile(r'(https://tech.hindustantimes.com/gaming/(news)/)')
    ...
    def get_content(self,i):
        ...
        for k in soup.find_all('div',{'class':re.compile(r'liveBlogWrapper entry__article htStoryDetail htStoryDetail_')}):
